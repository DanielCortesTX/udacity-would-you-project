      ...
      <div className="display-component">
        <div className="card test p-4 mb-4">
          ...
          <h2 className="display-4">{optionOne.text}</h2>
					<p className="lead">{optionOne.votes.length} out of {total} people voted for this. Or {calcPercent(optionOne.votes.length, total)}%
					</p>
        ...
        <div className={checkAnswerTwo(userAnswer)}>
          <h2 className="display-4">{optionTwo.text}</h2>
					<p className="lead">{optionTwo.votes.length} out of {total} people voted for this. Or {calcPercent(optionTwo.votes.length, total)}%
					</p>
          {!userAnswer && <p className="text-muted">Including you.</p>}
        </div>
      </div>
